...
// ─── Content packs (lazy-loaded expansion sources) ──────
// Packs are generated by data/fetch-packs.py into data/packs/*.js.
// The manifest is tiny and fetched at Database mount; each pack's data
// file is injected as a <script> only when the user enables it, then
// combined with the base SRD per-tab (base SRD is never mutated).
const PACKS_KEY = "dnd_codex_packs_v1";
const loadEnabledPacks = () => { try { return JSON.parse(localStorage.getItem(PACKS_KEY)) || []; } catch { return []; } };
...
const usePacks = () => {
  const [manifest, setManifest] = useState(window.CODEX_PACK_MANIFEST || null); // null = unknown yet
  const [enabled, setEnabled] = useState(loadEnabledPacks);
  const [loaded, setLoaded] = useState({});   // id -> data available on window.CODEX_PACKS
  const [loading, setLoading] = useState({}); // id -> script in flight

  useEffect(() => {
    if (manifest) return;
    injectScript("data/packs/manifest.js")
      .then(() => setManifest(window.CODEX_PACK_MANIFEST || []))
      .catch(() => setManifest([])); // no packs generated — sources UI shows a hint
  }, []);

  useEffect(() => { try { localStorage.setItem(PACKS_KEY, JSON.stringify(enabled)); } catch {} }, [enabled]);

  useEffect(() => {
    if (!manifest) return;
    enabled.forEach(id => {
      if (loaded[id] || loading[id]) return;
      if (window.CODEX_PACKS && window.CODEX_PACKS[id]) { setLoaded(l => ({ ...l, [id]: true })); return; }
      const p = manifest.find(m => m.id === id);
      if (!p) return; // stale id from localStorage — ignore
      setLoading(l => ({ ...l, [id]: true }));
      injectScript(p.file)
        .then(() => { setLoading(l => ({ ...l, [id]: false })); setLoaded(l => ({ ...l, [id]: true })); })
        .catch(() => setLoading(l => ({ ...l, [id]: false })));
    });
  }, [manifest, enabled]);

  const toggle = (id) => setEnabled(en => en.includes(id) ? en.filter(x => x !== id) : [...en, id]);
  return { manifest, enabled, loaded, loading, toggle };
};

// Search blobs are precomputed once per entry (WeakMap keyed by the entry
// object) so keystrokes don't re-stringify thousands of entries.
const __searchCache = new WeakMap();
const searchBlob = (e) => {
  let s = __searchCache.get(e);
  if (s === undefined) { s = JSON.stringify(e).toLowerCase(); __searchCache.set(e, s); }
  return s;
};
    ...
    <div className={"detail-body" + (hasImg ? " has-img" : "")}>
      ...
      <div>
        ...
        {m.desc && (
          <div className="detail-section">
            <h4>Lore</h4>
            <p style={{ whiteSpace: "pre-wrap" }}>{m.desc}</p>
          </div>
        )}
  ...
  return (
    <div className="detail-body">
      <div>
        ...
        {s.desc && <div className="detail-section"><p style={{ whiteSpace: "pre-wrap" }}>{s.desc}</p></div>}
        {s.higher_level && <div className="detail-section"><h4>At Higher Levels</h4><p style={{ whiteSpace: "pre-wrap" }}>{s.higher_level}</p></div>}
        {(s.classes || []).length > 0 && (
          <div className="detail-section">
            <h4>Classes</h4>
  ...
  return (
    <div className="detail-body">
      <div>
        ...
        {it.desc && <div className="detail-section"><p style={{ whiteSpace: "pre-wrap" }}>{it.desc}</p></div>}
      </div>
    </div>
  );
}
...
function FeatDetail({ f }) {
  return <div className="detail-body"><div>
    {f.prerequisite && <KV k="Prerequisite" v={f.prerequisite} />}
    <div className="detail-section"><p style={{ whiteSpace: "pre-wrap" }}>{f.desc || f.blurb}</p></div>
  </div></div>;
}
...
function DetailModal({ tab, entry, onClose, packLabel }) {
  useEffect(() => {
      ...
      <div className="detail-panel" role="dialog" aria-modal="true">
        <div className="detail-head">
          <div>
            ...
            {packLabel && <div className="detail-pack">{packLabel}</div>}
          </div>
...
function Database() {
  ...
  const { manifest, enabled, loaded, loading, toggle } = usePacks();

  const packsById = useMemo(() => {
    const m = {};
    (manifest || []).forEach(p => { m[p.id] = p; });
    return m;
  }, [manifest]);

  // Base SRD (curated + 5.1 bulk) + every enabled & loaded pack, per tab.
  const data = useMemo(() => {
    const base = SRD[tab] || [];
    const extra = [];
    for (const id of enabled) {
      if (!loaded[id]) continue;
      const pk = (window.CODEX_PACKS || {})[id];
      const rows = pk && pk.data && pk.data[tab];
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(e => searchBlob(e).includes(lo));
  }, [q, data]);

  useEffect(() => setShown(PAGE_FIRST), [tab, q, enabled]);
  const visible = filtered.slice(0, shown);

  const tabCount = (tid) => {
    let n = (SRD[tid] || []).length;
    for (const id of enabled) {
      const pk = loaded[id] && (window.CODEX_PACKS || {})[id];
      if (pk && pk.data[tid]) n += pk.data[tid].length;
      else if (packsById[id] && packsById[id].counts) n += packsById[id].counts[tid] || 0;
    }
    return n;
  };
  ...
  const groups = useMemo(() => {
    const g = [];
    (manifest || []).forEach(p => {
      let grp = g.find(x => x.name === p.group);
      if (!grp) { grp = { name: p.group, packs: [] }; g.push(grp); }
      grp.packs.push(p);
    });
    return g;
  }, [manifest]);
    ...
    <div className="db-shell" data-screen-label="Database">
      <div className="db-side">
        ...
        {DB_TABS.map(t => (
          <button key={t.id} className={"db-tab" + (tab === t.id ? " on" : "")} onClick={() => { setTab(t.id); setSelected(null); }}>
            <span>{t.label}</span>
            <span className="db-tab-count">{tabCount(t.id)}</span>
          </button>
        ))}
        <div className="db-side-head db-src-head">Sources</div>
        <div className="db-packs">
          <div className="db-pack base on" title="Always on — curated entries + the 5.1 SRD import.">
            <span className="db-pack-name">Core SRD 5.1 + curated</span>
            <span className="db-pack-badge-mini">5.1</span>
          </div>
          {manifest === null && <div className="muted small db-pack-note">Looking for packs…</div>}
          {manifest !== null && manifest.length === 0 && (
            <div className="muted small db-pack-note">No expansion packs found. Generate them with <code>python data/fetch-packs.py</code>.</div>
          )}
          {groups.map(g => (
            <div key={g.name} className="db-pack-group">
              <div className="db-pack-group-name">{g.name}</div>
              {g.packs.map(p => {
                const on = enabled.includes(p.id);
                const counts = Object.entries(p.counts || {}).map(([k, v]) => `${v} ${k}`).join(", ");
                return (
                  <button key={p.id} className={"db-pack" + (on ? " on" : "")} onClick={() => toggle(p.id)}
                    title={`${p.label} — ${counts} (${p.kb} KB)` + (p.license ? ` · License: ${p.license}` : "")}>
                    <span className="db-pack-name">{p.label}</span>
                    {loading[p.id] ? <span className="db-pack-spin">⟳</span> : <span className="db-pack-badge-mini">{p.badge}</span>}
                  </button>
                );
              })}
            </div>
        ...
        <div className="db-attrib">
          {SRD.attribution.short}
          {(manifest || []).length > 0 && (
            <div style={{ marginTop: 6 }}>
              Expansion packs served via <a href="https://open5e.com" target="_blank" rel="noreferrer">Open5e</a>;
              their content remains © its publishers under each document's license (see pack tooltips).
            </div>
          )}
        </div>
      </div>
      <div className="db-main">
        ...
        <div className="db-grid">
          {visible.map((e, i) => (
            <div key={(e.name || "") + i} className="db-card" onClick={() => setSelected(e)}>
              {e.pack && packsById[e.pack] && (
                <span className="db-card-src" title={packsById[e.pack].label}>{packsById[e.pack].badge}</span>
              )}
              {renderEntry(tab, e)}
            </div>
          ))}
        ...
        {filtered.length > shown && (
          <div className="db-more-row">
            <button className="ghost-btn" onClick={() => setShown(s => s + PAGE_STEP)}>
      ...
      {selected && (
        <DetailModal tab={tab} entry={selected} onClose={() => setSelected(null)}
          packLabel={selected.pack && packsById[selected.pack] ? packsById[selected.pack].label : null}/>
      )}
    </div>
  );
}
